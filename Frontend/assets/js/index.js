...
function create(){ 
    ...
    basetacho = this.physics.add.sprite(290, 490, 'basetacho').setImmovable();
    this.tacho = this.physics.add.sprite(290, 450, 'tachoverde').setScale(0.5);
    
    this.physics.add.collider(this.platform, basetacho);
    this.physics.add.collider(basura, basetacho);

    let monedas = 0;
    let monedasText;
    monedasText = this.add.text(16, 16, 'score:0', {
        fontSize:'32px',
        fill:'#000'
    })

    function collectBasura(basetacho, basura){
        basura.disableBody(true.true);
        monedas +=1;
        monedasText.setText('Monedas:' + monedas)
    }
